...
1. **The layering.** Your second brain is *who*. The team brain is *what*. You
...
This also answers the question someone always asks: *does the team brain replace
my second brain?* No. It's the layer underneath it.
...
person. Identity is a filter on the query (`requesting_user`), not a separate
store per teammate.

### 3. Everyone writes a connector (20 min, the headline made hands-on)

Attendees copy `team_brain/connectors/TEMPLATE.py`, point it at *their* source
...
`.mcp.json` is already wired. In Claude Code, call `search` / `who_knows`, watch
...
than conceptual. Point out what the server does *not* expose: there is no
...
exactly this and called them *LLM-free retrieval primitives*.
...
Frame these as *policy the substrate owns on everyone's behalf*, not as
...
- **Enrichment policy** (`enrich.py`): enrich-raw vs enriched on a Slack thread.
  The corpus stores what a thread *meant*, not its chatter.
...
### 6. Access control — the layer Cerebras skips (12 min, SECOND DIAGRAM)

Put up `TeamBrainSecurity.md`. Cerebras nails retrieval and leaves the lock
unspecified — their only visibility construct, "projects," is a relevance scope,
not enforcement. A shared brain with no access layer is a leak generator.

Teach the two steps (keep them separate — this is the distinction that clicks):
...
> is code, not instructions — and the *server* owns identity, not the agent.
...
Cerebras-style "Projects" turned into *enforced* grants, `who_knows` for org
