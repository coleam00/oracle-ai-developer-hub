...
This is the other half people miss. The team brain is not a passive store you
...
```
  ...
  (her agent, her SOUL,       (his agent, his           (no personality
...
Note the third caller. Cerebras's system serves humans, automations, **and**
...
The workshop build enforced permissions with a `WHERE` clause the application had to remember on every read path. That works until someone holds the database credential, at which point the clause is optional. In this edition the policy is attached to the table: a `DBMS_RLS` row policy reads the caller's identity from an application context that only the `tb_session` package can write, and the kernel appends the predicate to every `SELECT`, whether it comes from the MCP server, the LangChain agent, or a SQL client. A session that never established an identity sees zero rows. The substrate still owns the policy; it just stopped trusting the application to apply it.
